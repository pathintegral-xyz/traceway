...
func (r *notificationRuleRepository) FindEnabledPolledRules(tx lit.Executor) ([]*models.NotificationRuleWithChannel, error) {
	return lit.Select[models.NotificationRuleWithChannel](
		...
		WHERE r.enabled = true AND c.enabled = true
			AND r.rule_type NOT IN ('new_error', 'error_regression', 'ai_trace_cost', 'ai_conversation_cost', 'ai_flagged_content', 'check_down')`,
	)
}
...
func (r *notificationRuleRepository) FindEnabledEventRules(tx lit.Executor, projectId uuid.UUID) ([]*models.NotificationRuleWithChannel, error) {
	return lit.SelectNamed[models.NotificationRuleWithChannel](
		...
		WHERE r.project_id = :project_id AND r.enabled = true AND c.enabled = true
			AND r.rule_type IN ('new_error', 'error_regression', 'ai_trace_cost', 'ai_conversation_cost', 'ai_flagged_content', 'check_down')`,
		lit.P{"project_id": projectId},
	)
}
