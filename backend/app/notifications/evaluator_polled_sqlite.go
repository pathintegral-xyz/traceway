...
func evaluateErrorCountThreshold(ctx context.Context, rule *models.NotificationRule, projectId uuid.UUID) (*EvalResult, error) {
	...
	var count int64
	err := db.TelemetryDB.QueryRowContext(ctx,
		"SELECT COUNT(*) FROM exception_stack_traces WHERE project_id = ? AND recorded_at >= ? AND recorded_at <= ? AND CAST(is_message AS TEXT) IN ('0', 'false')",
...
func computeImpactEndpoints(ctx context.Context, projectId uuid.UUID, minRequests int) ([]impactEndpointData, error) {
	...
	p99Query := `SELECT endpoint, duration FROM (
		...
		FROM endpoints WHERE project_id = ? AND recorded_at >= ? AND recorded_at <= ? AND CAST(is_stream AS TEXT) IN ('0', 'false')
