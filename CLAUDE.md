...
The fork keeps `cloudflare-integration.yml` on pushes and pull requests targeting `cloudflare`, and `cloudflare-deploy.yml` deploys stage on pushes to that branch. The inherited backend, CLI, frontend, and vulnerability workflows are manual-only through `workflow_dispatch`; they no longer run on `main`, PR labels, or daily schedules. The inherited CLI/Helm release, docs/website deployment, and auto-fix workflows are also manual-only. Their dispatches run real publishing, deployment, or agent actions where applicable.

Action versions are watched by `.github/dependabot.yml` (the `github-actions` ecosystem only; npm and Go modules are deliberately not watched there). It opens a weekly PR per group — `actions/*` in one, every third-party action in the other. Those PRs do not run the manual-only inherited CI. Run a relevant workflow from the Actions tab when reviewing an action bump. `workflow_dispatch` is **not** a dry run on the release path: `release-docs.yml` and `release-website.yml` deploy to Cloudflare, other `release-*.yml` workflows publish real assets, and `traceway-autofix.yml` runs the agent for real against an existing issue. Validate those action changes by reading release notes, and dispatch only where the target is disposable. `traceway-autofix.yml` calls the reusable `autofix.yml` workflow; `docs/pages/learn/auto-fix.mdx` documents its contract. Every updatable pin is a bare major tag, so Dependabot only ever proposes major bumps here.

The inherited `plugin-scanner.yml` follows the same manual-only policy. It uses `.plugin-scanner.toml` for reviewed ignore paths and pins its actions to commit SHAs.
