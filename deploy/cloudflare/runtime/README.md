...
This directory is a deployable stage template, not an instruction to deploy production. Copy `wrangler.jsonc.example` to `wrangler.jsonc`, replace every `replace-with-*` value, then run `npm run check:runtime` before using Wrangler.
...
1. Create two empty D1 databases named `traceway-main-stage` and `traceway-telemetry-stage`; copy both database UUIDs into the stage config.
